...
It might also be interesting to look at the number of unique letters in a word. I'll group the words into bins which have the same values for: unique letter count, word length, M value, and count the number of words in each bin.
...
Interpretation: it's messy! But I can see that low M numbers - 0 to 3 or so - cluster where words are long, and unique letters in a word are around 6 to 9. There are an interesting outlier with 16 unique letters (position="jitter" might place the point not-quite-at 16).
...
I didn't know 'zzz' was a word! Its M value is 49 - meaning it has 0 letters in common with 49 out of 50 states. Which state does it have a letter in common with?
```{r}
